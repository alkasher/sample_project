...
tms::gf2poly::generate_irrpolys(unsigned int const amount,
					unsigned int const max_defect)//принимает s и m
{													// сумма степеней многочленов должна быть меньше либо равна m
	...
	auto number_to_poly = [&](uintmax_t coeffs_number) -> irrpoly::gfpoly {//преобразование числа в двоичную запись, а затем из двоичной записи строится полином
