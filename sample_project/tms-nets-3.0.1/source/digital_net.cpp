...
namespace tms
{
	...
	Point
		DigitalNet::generate_point2(CountInt pos, int dimension) const
	{
		//std::cout << "DERgen called\n";
		IntPoint int_point(m_dim);//using IntPoint   = std::vector<uintmax_t>;
		store_int_point2(int_point, pos, dimension);
	...
	void
		DigitalNet::store_int_point2(IntPoint& point,
			CountInt  pos, int dimension) const
	{
		for (BasicInt i = 0; i < m_dim; ++i)
		{
			point[i] = 0;
	...
	p_DigitalNet::p_DigitalNet(int nbits, int dim, int _p) : m_dim(dim), m_nbits(nbits), p(_p){
	}

	std::vector<std::vector<std::vector<int>>> p_DigitalNet::get_matrix() {
		return matrix;
	}
